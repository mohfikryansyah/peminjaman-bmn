...
@section('content')
                    ...
                    <tr class="bg-white border-b">
                        ...
                        <td class="px-6 py-4">
                            <label class="relative inline-flex items-center mr-5 cursor-pointer">
                                <input type="checkbox" id="verified_{{ $user->id }}" data-user-id="{{ $user->id }}" class="sr-only peer" {{ $user->verified ? 'checked' : '' }} onclick="toggleUserStatus(this)">
    ...
    <script>
        function toggleUserStatus(checkbox) {
            ...
            var data = JSON.stringify({ isActive: isActive });
            xhr.send(data);
        }
    </script>
